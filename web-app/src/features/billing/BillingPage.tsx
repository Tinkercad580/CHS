    ...
    <div style={{ animation: "fadeUp .3s cubic-bezier(.2,.7,.3,1)" }}>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 20, flexWrap: "wrap", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 9, flexWrap: "wrap" }}>
          <button type="button" onClick={() => setRecomputeOpen(true)} style={secondaryBtn}>Recompute</button>
          <button type="button" onClick={() => setPublishOpen(true)} style={primaryBtn}>Publish 248 bills</button>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: 14 }}>
        <div style={card}>
          ...
          {CHARGE_HEADS.map((h) => (
            <div key={h.name} style={{ padding: "12px 20px", borderBottom: "1px solid var(--border-soft,#F1F4F3)", display: "flex", gap: 14, alignItems: "center" }}>
          ...
          <div style={{ ...card, padding: 20 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {BILLING_EXCEPTIONS.map((ex) => (
                <div key={ex.title} style={{ padding: "12px 13px", border: "1px solid var(--warn-border,#F5DFBE)", background: "var(--warn-wash,#FDF9F3)", borderRadius: 11 }}>
