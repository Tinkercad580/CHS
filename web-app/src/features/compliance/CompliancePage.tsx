  ...
  return (
    <div style={{ animation: "fadeUp .3s cubic-bezier(.2,.7,.3,1)" }}>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 20, flexWrap: "wrap", marginBottom: 20 }}>
        ...
        <button type="button" onClick={() => setRegistrarOpen(true)} className="press-scale" style={primaryBtn}>Registrar report</button>
      </div>
      ...
      <div style={{ background: "var(--info-wash,#EAF0FE)", border: "1px solid var(--info-border,#C7D7FB)", borderRadius: 14, padding: "15px 18px", display: "flex", gap: 12, marginBottom: 16 }}>
        ...
        <button type="button" onClick={() => setFixOpen(true)} className="press-scale" style={{ alignSelf: "center", flex: "none", height: 34, padding: "0 14px", border: 0, borderRadius: 9, background: "#1D4ED8", color: "#fff", font: "600 12.5px/1 Figtree, sans-serif", cursor: "pointer" }}>
          Apply fix
        </button>
      </div>
      ...
      <div style={{ background: "var(--surface,#fff)", border: "1px solid var(--border,#E3E9E6)", borderRadius: 15, overflow: "hidden" }}>
        {COMPLIANCE_ITEMS.map((c, i) => (
          <div key={c.item} className="row-hover" style={{ padding: "14px 20px", borderBottom: i < COMPLIANCE_ITEMS.length - 1 ? "1px solid var(--border-soft,#F1F4F3)" : "none", display: "flex", gap: 16, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <span className="press-scale" style={{ flex: "none", padding: "4px 11px", borderRadius: 8, background: "var(--subtle,#F1F4F3)", font: "600 12px/1.5 Figtree, sans-serif", color: "var(--ink-soft,#3D4A46)", cursor: "pointer" }}>{c.action}</span>
          </div>
        ))}
      </div>
