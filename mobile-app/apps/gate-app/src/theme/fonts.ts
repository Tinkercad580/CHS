...
/**
 * Loads exactly the weights the design uses (README.md "Typography"). IBM Plex Mono
 * 700 isn't listed in that font-weight line, but the type scale's `gateCodeDisplay`
 * and the prototype's own code boxes render mono at weight 700 — loaded here to
 * match what is actually on screen rather than the (incomplete) weight list.
 */
export function useGateFonts() {
  return useFonts({
    Figtree_400Regular,
    Figtree_500Medium,
    Figtree_600SemiBold,
